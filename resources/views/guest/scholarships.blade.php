        ...
        <div class="container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2 small">
                    ...
                    <li class="breadcrumb-item active text-navy fw-semibold" aria-current="page">Scholarships</li>
                </ol>
            </nav>
            <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mt-3">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h2 class="fw-bold text-navy mb-1">Scholarships & Grants</h2>
                        <p class="text-secondary mb-0">Explore financial support, merit-based scholarships, and research grants.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row justify-content-center mb-5">
            <div class="col-md-8">
                <div class="card card-premium p-3">
                    <form action="{{ route('guest.scholarships') }}" method="GET" class="d-flex">
                        <div class="input-group">
                            <span class="input-group-text bg-light border-end-0 text-secondary"><i class="bi bi-search"></i></span>
                            <input type="text" name="search" class="form-control bg-light border-start-0 py-2 ps-0" placeholder="Search scholarships by title or keywords..." value="{{ $search }}">
            ...
            @forelse($scholarships as $scholarship)
                <div class="col-md-4">
                    <div class="card card-premium p-4 h-100 d-flex flex-column justify-content-between">
                        <div>
                            <span class="badge bg-primary-subtle text-primary fw-semibold px-2 py-1 mb-3"><i class="bi bi-tag-fill me-1"></i>{{ $scholarship->category->name ?? 'Grant' }}</span>
                            <h5 class="fw-bold text-navy mb-2">{{ Str::limit($scholarship->title, 50) }}</h5>
                            <p class="text-secondary small line-clamp-3 mb-4" style="line-height: 1.6;">
                                {{ Str::limit($scholarship->description, 140) }}
                            </p>
                            <div class="mb-3">
                                <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1"><i class="bi bi-cash-stack me-1"></i>Amount: {{ $scholarship->amount ?: 'Varies' }}</span>
                            </div>
                        </div>
                        <div class="mt-3 pt-3 border-top d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('guest.scholarship.detail', $scholarship->id) }}" class="btn btn-premium-outline btn-sm rounded-pill px-3">View Details</a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-12 text-center py-5 my-5">
                    ...
                    <h5 class="fw-bold text-navy mb-2">No Scholarships Found</h5>
                    <p class="text-secondary">No active scholarships found matching your search query.</p>
                </div>
            @endforelse
        </div>
